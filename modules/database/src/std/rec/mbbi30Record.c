...
#define NUM_STATES (mbbi30RecordST29 - mbbi30RecordST00 + 1)
...
epicsExportAddress(rset,mbbi30RSET);

static void checkAlarms(mbbi30Record *, epicsTimeStamp *);
static void monitor(mbbi30Record *);
static long readValue(mbbi30Record *);

static void init_common(mbbi30Record *prec)
{
    epicsUInt32 *pstate_values = &prec->vl00;
    char *pstate_string = prec->st00;
    int i;

    /* Check if any states are defined */
    for (i = 0; i < NUM_STATES; i++, pstate_string += sizeof(prec->st00)) {
...
{
    struct mbbi30Record *prec = (struct mbbi30Record *)pcommon;
    mbbi30dset  *pdset = (mbbi30dset *) prec->dset;
    long status = 0;

    if (pass == 0) return 0;

    if (!pdset) {
        recGblRecordError(S_dev_noDSET, prec, "mbbi30: init_record");
        return S_dev_noDSET;
    }

    if ((pdset->common.number < 5) || (pdset->read_mbbi30 == NULL)) {
        recGblRecordError(S_dev_missingSup, prec, "mbbi30: init_record");
...
static long process(struct dbCommon *pcommon)
{
    ...
    if ((pdset == NULL) || (pdset->read_mbbi30 == NULL)) {
        prec->pact = TRUE;
        recGblRecordError(S_dev_missingSup, prec, "read_mbbi30");
        return S_dev_missingSup;
    }
...
{
    mbbi30Record *prec = (mbbi30Record *) paddr->precord;
    int fieldIndex = dbGetFieldIndex(paddr);

    switch (paddr->special) {
    case SPC_MOD:
        if (fieldIndex == mbbi30RecordSIMM) {
            if (!after)
        ...
        if (!after)
            return 0;
        init_common(prec);
        /* Note: VL00..VL20 are also SPC_MOD */
        if (fieldIndex >= mbbi30RecordST00 && fieldIndex <= mbbi30RecordST29
                && prec->val == fieldIndex - mbbi30RecordST00) {
            db_post_events(prec, &prec->val, DBE_VALUE | DBE_LOG);
        }
        return 0;

    default:
        recGblDbaddrError(S_db_badChoice, paddr, "mbbi30: special");
...
static long get_enum_str(const DBADDR *paddr, char *pstring)
{
    ...
    if (index != mbbi30RecordVAL) {
        strcpy(pstring, "Illegal_Value");
    }
    else if (val < NUM_STATES) {
        char *pstate = prec->st00 + val * sizeof(prec->st00);

        strncpy(pstring, pstate, sizeof(prec->st00));
    }
    else {
...
{
    mbbi30Record *prec = (mbbi30Record *) paddr->precord;
    char *pstate = prec->st00;
    const int choices = (NUM_STATES<DB_MAX_CHOICES) ? NUM_STATES:DB_MAX_CHOICES;
    int i;
    short states = 0;

    memset(pes->strs, '\0', sizeof(pes->strs));
    for (i = 0; i < choices; i++, pstate += sizeof(prec->st00) ) {
        strncpy(pes->strs[i], pstate, sizeof(prec->st00));
...
static long put_enum_str(const DBADDR *paddr, const char *pstring)
{
    ...
    if (prec->sdef) {
        pstate = prec->st00;
        for (i = 0; i < NUM_STATES; i++) {
            if (strncmp(pstate, pstring, sizeof(prec->st00)) == 0) {
                prec->val = i;
                prec->udf =  FALSE;
                return 0;
            }
            pstate += sizeof(prec->st00);
        }
    }
    return S_db_badChoice;
}

static void checkAlarms(mbbi30Record *prec, epicsTimeStamp *timeLast)
{
    double aftc, afvl;
    ...
    if (val >= NUM_STATES) {
        ...
        epicsEnum16 *severities = &prec->sv00;
...
static void monitor(mbbi30Record *prec)
{
...
static long readValue(mbbi30Record *prec)
{
    mbbi30dset *pdset = (mbbi30dset *) prec->dset;
    long status = 0;
